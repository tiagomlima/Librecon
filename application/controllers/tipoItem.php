...
class TipoItem extends CI_Controller {
	...
	function gerenciar(){
        ...
        $config['total_rows'] = $this->tipoItem_model->count('tipoItem');
        $config['per_page'] = 10;
	    ...
	    $this->data['results'] = $this->tipoItem_model->get('tipoItem','idTipoItem,nomeTipoItem,disciplina','',$config['per_page'],$this->uri->segment(3));
    ...
    function adicionar() {
        ...
        if ($this->form_validation->run('tipoItem') == false) {
            ...
            if ($this->tipoItem_model->add('tipoItem', $data) == TRUE) {
